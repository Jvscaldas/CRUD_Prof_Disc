...
public class PrincipalController {
    ...
    public void acaoProfessor(ActionEvent event) {
        ...
        ProfessorController professorController =
                new ProfessorController(tfCodigoProfessor, tfNomeProfessor, tfTitulacaoProfessor, taListaProfessores);

        if (cmd.contains("Inserir") || (cmd.contains("Atualizar"))
                && tfCodigoProfessor.getText().isEmpty()
                && tfNomeProfessor.getText().isEmpty()
                && tfTitulacaoProfessor.getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Preencha os campos", "ERRO"
                    , JOptionPane.ERROR_MESSAGE);
        } else {
            if (cmd.contains("Excluir") || cmd.contains("Buscar") || cmd.contains("tfCodigoProfessor")
                    && tfCodigoProfessor.getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Preencha o código", "ERRO"
                        , JOptionPane.ERROR_MESSAGE);
            } else {
                try {
    ...
    @FXML
    void acaoDisciplina(ActionEvent event) {
